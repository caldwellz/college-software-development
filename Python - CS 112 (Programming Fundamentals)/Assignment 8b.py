...
#Description: dups() & sum()


def dups(numlist):
	duplist = []
	for i in numlist:
		if numlist[i] in numlist[i+1:] and numlist[i] not in duplist:
			duplist = duplist + [numlist[i]]
...
def main(): #main function needed in all programs for automated testing
	while True:
		...
		print("The following numbers were duplicated in the list:", dups(numlist))
