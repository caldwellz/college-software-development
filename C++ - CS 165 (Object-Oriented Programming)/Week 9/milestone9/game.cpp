...
Bird* Game :: createBird()
{
   ...
   // Set a random position along the left side
   int birdY = random(-190, 190);
   Point birdPos(-190, birdY);

   // Randomize the bird type
   // - Skew the probability just a little to make things interesting
   int birdNum = random(0, 6);
   if (birdNum <= 2)            // Chance: 1/2 - Bird
   {
      newBird = new Bird(birdPos);
      // Match vertical velocity to Y position
      if (birdY > 0)
         birdVel.setDy(random(-4.0d, 0.0d));
      else
         birdVel.setDy(random(0.0d, 4.0d));
      birdVel.setDx(random(3.0d, 6.0d));
   }
   else if (birdNum <= 4)               // 1/3 - Tough Bird
   {
      newBird = new ToughBird(birdPos);
      // Match vertical velocity to Y position
      if (birdY > 0)
         birdVel.setDy(random(-3.0d, 0.0d));
      else
         birdVel.setDy(random(0.0d, 3.0d));
      birdVel.setDx(random(2.0d, 4.0d));
   }
   else                                 // 1/6 - Sacred Bird
   {
      newBird = new SacredBird(birdPos);
      // Match vertical velocity to Y position
      if (birdY > 0)
         birdVel.setDy(random(-4.0d, 0.0d));
      else
