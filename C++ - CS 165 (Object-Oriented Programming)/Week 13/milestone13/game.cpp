...
Game :: ~Game()
{
   // Clean up any remaining asteroids
...
/**************************************************************************
 * GAME :: ADVANCE SHIP
 * Advance the ship per game rules if it is alive; wait and quit if not
 **************************************************************************/
void Game::advanceShip()
{
   ...
   if (ship.isAlive()) {
      ...
      if (deadFrames >= 75) {
         // According to the Interwebs, exit() won't unwind / call destructors,
         // but supposedly exceptions do, so throw one to get back out to main
         throw NormalExit();
      } else {
         deadFrames++;
      }
...
void Game::createRocks()
{
   ...
   Velocity asterVel;
   Point asterPt;
   Point shipPt = ship.getPoint();
   int angle;
   int minX = topLeft.getX() - OFF_SCREEN_BORDER_AMOUNT + 1;
   int maxX = bottomRight.getX() + OFF_SCREEN_BORDER_AMOUNT - 1;
   int maxY = topLeft.getY() + OFF_SCREEN_BORDER_AMOUNT - 1;
   int minY = bottomRight.getY() - OFF_SCREEN_BORDER_AMOUNT + 1;
   int shipDist;
   ...
   for (int i = 0; i < START_BIG_ASTEROIDS; ++i)
   {
      ...
      // Generate a random position, using an appropriate
      // buffer around the ship to prevent insta-kill.
      do {
         asterPt.setX(random(minX, maxX));
         asterPt.setY(random(minY, maxY));
         newRock->setPoint(asterPt);
         shipDist = getClosestDistance(ship, *newRock);
      } while (shipDist < SHIP_ROCKGEN_BUFFER);
...
void Game :: handleCollisions()
{
   ...
   for (int n = 0; n < asteroids.size(); n++) {
      if (asteroids[n]->isAlive()) {
         // Check for collision with ship
         if (ship.isAlive()) {
            if (getClosestDistance(*asteroids[n], ship) <= (ship.getRadius() + asteroids[n]->getRadius())) {
               ship.kill();
               asteroids[n]->breakRock(asteroids);
            }
         }
         ...
         for (int i = 0; i < bullets.size(); i++) {
            if (bullets[i].isAlive()) {
               ...
               if (getClosestDistance(*asteroids[n], bullets[i]) <= (bullets[i].getRadius() + asteroids[n]->getRadius())) {
                  bullets[i].kill();
                  score += asteroids[n]->breakRock(asteroids);
               }
            }
         }
      }
